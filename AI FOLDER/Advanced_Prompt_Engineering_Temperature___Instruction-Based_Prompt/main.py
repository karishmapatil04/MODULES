...
def generate_response(prompt, temperature=0.5):
    """Generate a response with the correct model ID and error handling."""
    # This is the standard model ID for early 2026
    model_id = "gemini-2.0-flash" 
    
    try:
        client = genai.Client(api_key=config.GEMINI_API_KEY)
        
        # Adding a small delay here helps prevent 429 errors before they happen
        time.sleep(2) 
        
        response = client.models.generate_content(
            model=model_id,
            contents=prompt, # Simplified syntax supported by newer SDK
        ...
        # If it's a quota error, tell the user to wait
        if "429" in str(e):
